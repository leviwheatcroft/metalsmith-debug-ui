...
function plugin () {
  return async function debugUi (files, metalsmith) {
    const _metalsmith = Metalsmith(metalsmith._directory)

    _metalsmith
      .metadata(metalsmith._metadata)
      .source(metalsmith._source)
      .destination(metalsmith._destination)
      .concurrency(metalsmith._concurrency)
      .clean(metalsmith._clean)
      .frontmatter(metalsmith._frontmatter)
      .ignore(metalsmith.ignores)
      .env(metalsmith.env())

    dbg('patching plugins')
    metalsmith.plugins.forEach((plugin) => {
      if (plugin === debugUi) return
      const patchedPlugin = async (files, metalsmith) => {
      ...
      _metalsmith.use(patchedPlugin)
    })

    const _writeClient = writeClient
    _metalsmith.use(async function writeClient (files, metalsmith) {
      await _writeClient(files, metalsmith)
    })
    ...
    await deferred
    dbg('plugin done')
    return Promise.reject(new Error('exit original queue'))
  }
}
...
async function writeClient (files, metalsmith) {
  const clientFileNames = await readdir(join(__dirname, 'build', 'debug-ui'))
  const clientFiles = {}
  for await (const fileName of clientFileNames) {
    const readPath = join(__dirname, 'build', 'debug-ui', fileName)
 ...
 * ## exports
 */
module.exports = { patch, report, clear, plugin }
